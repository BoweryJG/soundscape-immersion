export const soundscapes = [
  // Ocean & Water (10 files)
  {
    id: 1,
    name: "Ocean Waves Crashing",
    category: "ocean",
    file: "/sounds/ocean/ocean_waves_crashing.wav",
    description: "Powerful ocean waves crashing on a rocky shore",
    color: "#0077be",
    visualization: "waveforms"
  },
  {
    id: 2,
    name: "Gentle Ocean Waves",
    category: "ocean", 
    file: "/sounds/ocean/gentle_ocean_waves.wav",
    description: "Soft, rhythmic waves lapping the shore",
    color: "#87ceeb",
    visualization: "gentleWaves"
  },
  {
    id: 3,
    name: "Gentle Ocean",
    category: "ocean",
    file: "/sounds/ocean/gentle-ocean.mp3", 
    description: "Deep ocean meditation with whale songs",
    color: "#4682b4",
    visualization: "underwater"
  },
  {
    id: 4,
    name: "Ocean Nature Sounds",
    category: "ocean",
    file: "/sounds/ocean/ocean-waves.mp3",
    description: "Natural ocean soundscape for relaxation",
    color: "#5f9ea0",
    visualization: "oceanDepth"
  },
  {
    id: 5,
    name: "Underwater Ambience",
    category: "ocean",
    file: "/sounds/ocean/underwater_ambience.wav",
    description: "Mysterious underwater world sounds",
    color: "#008b8b",
    visualization: "underwater"
  },
  {
    id: 6,
    name: "Mountain Stream",
    category: "ocean", 
    file: "/sounds/ocean/mountain_stream.wav",
    description: "Crystal clear mountain stream flowing",
    color: "#40e0d0",
    visualization: "stream"
  },
  {
    id: 7,
    name: "Waterfall Forest",
    category: "ocean",
    file: "/sounds/ocean/waterfall_forest.wav", 
    description: "Majestic waterfall in forest setting",
    color: "#20b2aa",
    visualization: "waterfall"
  },
  {
    id: 8,
    name: "Rain on Lake",
    category: "ocean",
    file: "/sounds/ocean/rain_on_lake.wav",
    description: "Gentle rain drops on calm lake surface",
    color: "#778899",
    visualization: "rainDrops"
  },
  {
    id: 9,
    name: "River Flowing",
    category: "ocean",
    file: "/sounds/ocean/river_flowing.wav",
    description: "Peaceful river flowing through nature",
    color: "#6495ed",
    visualization: "riverFlow"
  },
  {
    id: 10,
    name: "Tropical Beach",
    category: "ocean",
    file: "/sounds/ocean/tropical_beach.wav",
    description: "Exotic tropical beach with palm trees",
    color: "#00ced1",
    visualization: "tropical"
  },
  
  // Forest & Nature (9 files)
  {
    id: 11,
    name: "Rainforest Dawn Chorus", 
    category: "forest",
    file: "/sounds/forest/rainforest_dawn_chorus.mp3",
    description: "Awakening rainforest with bird symphony",
    color: "#228b22",
    visualization: "forestDawn"
  },
  {
    id: 12,
    name: "Forest Birds Singing",
    category: "forest",
    file: "/sounds/forest/forest_birds_singing.mp3", 
    description: "Melodic forest birds in harmony",
    color: "#32cd32",
    visualization: "birdChorus"
  },
  {
    id: 13,
    name: "Forest Birds",
    category: "forest",
    file: "/sounds/forest/forest-birds.mp3",
    description: "Natural forest bird ambience",
    color: "#90ee90",
    visualization: "forestBirds"
  },
  {
    id: 14,
    name: "Wind Through Trees",
    category: "forest", 
    file: "/sounds/forest/wind_through_trees.mp3",
    description: "Gentle wind rustling through leaves",
    color: "#9acd32",
    visualization: "windTrees"
  },
  {
    id: 15,
    name: "Deep Forest Ambience",
    category: "forest",
    file: "/sounds/forest/deep_forest_ambience.mp3",
    description: "Mysterious deep forest atmosphere", 
    color: "#006400",
    visualization: "deepForest"
  },
  {
    id: 16,
    name: "Forest Creek Babbling",
    category: "forest",
    file: "/sounds/forest/forest_creek_babbling.mp3",
    description: "Babbling brook through forest glade",
    color: "#2e8b57",
    visualization: "forestCreek"
  },
  {
    id: 17,
    name: "Night Forest Sounds",
    category: "forest",
    file: "/sounds/forest/night_forest_sounds.mp3", 
    description: "Peaceful night forest with crickets",
    color: "#1c5f3e",
    visualization: "nightForest"
  },
  {
    id: 18,
    name: "Bamboo Forest Wind",
    category: "forest",
    file: "/sounds/forest/bamboo_forest_wind.ogg",
    description: "Zen bamboo forest with gentle breeze",
    color: "#7cfc00",
    visualization: "bambooForest"
  },
  {
    id: 19,
    name: "White Noise Sleep",
    category: "forest",
    file: "/sounds/forest/white_noise_sleep.mp3",
    description: "Calming white noise for deep sleep",
    color: "#98fb98",
    visualization: "calmingSleep"
  },

  // Weather & Storm (8 files)
  {
    id: 20,
    name: "Thunderstorm Heavy Rain",
    category: "weather",
    file: "/sounds/weather/01-thunderstorm-heavy-rain.mp3",
    description: "Intense thunderstorm with heavy rainfall",
    color: "#483d8b",
    visualization: "thunderstorm"
  },
  {
    id: 21,
    name: "Gentle Spring Rain", 
    category: "weather",
    file: "/sounds/weather/02-gentle-spring-rain.mp3",
    description: "Soft spring rain on fresh leaves",
    color: "#87ceeb",
    visualization: "springRain"
  },
  {
    id: 22,
    name: "Rain on Tent",
    category: "weather",
    file: "/sounds/weather/03-rain-on-tent.mp3",
    description: "Cozy rain pattering on canvas tent",
    color: "#708090",
    visualization: "rainTent"
  },
  {
    id: 23,
    name: "Distant Rolling Thunder",
    category: "weather", 
    file: "/sounds/weather/04-distant-rolling-thunder.mp3",
    description: "Distant thunder rolling across sky",
    color: "#2f4f4f",
    visualization: "distantThunder"
  },
  {
    id: 24,
    name: "Heavy Rain on Roof",
    category: "weather",
    file: "/sounds/weather/05-heavy-rain-on-roof.mp3", 
    description: "Heavy rain drumming on metal roof",
    color: "#696969",
    visualization: "rainRoof"
  },
  {
    id: 25,
    name: "Wind Storm",
    category: "weather",
    file: "/sounds/weather/06-wind-storm.mp3",
    description: "Powerful wind storm howling",
    color: "#778899",
    visualization: "windStorm"
  },
  {
    id: 26,
    name: "Snow Falling Peacefully", 
    category: "weather",
    file: "/sounds/weather/07-snow-falling-peacefully.mp3",
    description: "Peaceful snowfall in quiet forest",
    color: "#f0f8ff",
    visualization: "snowfall"
  },
  {
    id: 27,
    name: "Hailstorm on Metal",
    category: "weather",
    file: "/sounds/weather/08-hailstorm-on-metal.mp3",
    description: "Dramatic hailstorm on metal surfaces",
    color: "#b0c4de",
    visualization: "hailstorm"
  },

  // Meditation & Spiritual (2 files)
  {
    id: 28,
    name: "Tibetan Singing Bowls",
    category: "meditation",
    file: "/sounds/meditation/singing-bowls.mp3",
    description: "Sacred Tibetan singing bowls meditation",
    color: "#dda0dd",
    visualization: "singingBowls"
  },
  {
    id: 29,
    name: "Om Chanting Monks",
    category: "meditation", 
    file: "/sounds/meditation/om_chanting_monks.mp3",
    description: "Deep Om chanting by Tibetan monks",
    color: "#9370db",
    visualization: "sacredChant"
  },

  // Ambient & ASMR (1 file)
  {
    id: 30,
    name: "Crackling Fireplace",
    category: "ambient",
    file: "/sounds/ambient/crackling_fireplace.mp3",
    description: "Cozy fireplace with crackling logs",
    color: "#ff6347",
    visualization: "fireplace"
  }
];

export const categories = {
  ocean: {
    name: "Ocean & Water",
    description: "Immersive water soundscapes",
    color: "#0077be",
    count: 10
  },
  forest: {
    name: "Forest & Nature", 
    description: "Deep forest and nature sounds",
    color: "#228b22",
    count: 9
  },
  weather: {
    name: "Weather & Storm",
    description: "Rain, thunder, and weather phenomena",
    color: "#483d8b", 
    count: 8
  },
  meditation: {
    name: "Meditation & Spiritual",
    description: "Sacred sounds for inner peace",
    color: "#9370db",
    count: 2
  },
  ambient: {
    name: "Ambient & ASMR",
    description: "Calming ambient environments", 
    color: "#ff6347",
    count: 1
  }
};

export const getSoundscapesByCategory = (category) => {
  return soundscapes.filter(sound => sound.category === category);
};

export const getSoundscapeById = (id) => {
  return soundscapes.find(sound => sound.id === id);
};